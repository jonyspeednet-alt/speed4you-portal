      ...
      <section style={{ ...styles.hero, ...(isMobile ? styles.heroMobile : isTablet ? styles.heroTablet : {}) }}>
        ...
        <div style={{ ...styles.commandCard, ...(isMobile ? styles.commandCardMobile : {}) }}>
          <div style={styles.searchBox}>
            ...
            {query && (
              <button type="button" onClick={resetSearch} style={styles.clearBtn}>
                Clear
              </button>
            )}
          </div>
...
const styles = {
  ...
  card: { textDecoration: 'none', display: 'grid', gap: '8px' },
  posterWrapper: {
    position: 'relative',
    borderRadius: '16px',
    overflow: 'hidden',
  ...
  info: { padding: '0 2px', display: 'grid', gap: '4px' },
  infoTop: { display: 'flex', justifyContent: 'space-between', gap: '10px', alignItems: 'flex-start' },
  cardTitle: { fontSize: '0.88rem', color: 'var(--text-primary)', fontWeight: '700', lineHeight: '1.3', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' },
  cardMeta: { fontSize: '0.72rem', color: 'var(--text-muted)' },
  languageMeta: { fontSize: '0.7rem', color: 'var(--accent-secondary)' },
};
